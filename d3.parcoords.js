d3.parcoords = function (config) {
  ...
  pc.colorBySegment = function (callback) {
    __.colorBySegment = callback
  }
  ...
  function single_curve(d, ctx) {
    ...
    console.log('single curve initial moveto', cps[0].e(1), cps[0].e(2))
  ...
  function single_path(d, ctx) {
    let prevKey = d3.entries(__.dimensions)[0].key
    d3.entries(__.dimensions).slice(1, d3.entries(__.dimensions).length)
      .forEach(function (p, i) {
        ctx.beginPath()
        ctx.moveTo(position(prevKey), __.dimensions[prevKey].yscale(d[prevKey]))
        if (typeof __.colorBySegment === 'function') {
          ctx.strokeStyle = __.colorBySegment(prevKey, d[prevKey], p.key, d[p.key])
        }
        prevKey = p.key

        ctx.lineTo(position(p.key), typeof d[p.key] == 'undefined' ?
          getNullPosition() : __.dimensions[p.key].yscale(d[p.key]));
